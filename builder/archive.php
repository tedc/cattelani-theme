...
?>
<div class="projects projects--grow-lg" post-type-archive post-type="<?php the_sub_field('post_type'); ?>" lang="<?php echo $sitepress->get_current_language(); ?>">
	<div class="projects__filters projects__filters--gray projects__filters--grow-md projects__filters--grid"> 
		<div class="projects__select" click-outside="isSelect['collezioni']=false" ng-class="{'projects__select--filled' : projects.collezioni}" ng-click="$event.stopPropagation();isSelect['collezioni']=!isSelect['collezioni']">
			<span class="projects__value" ng-bind-html="(projects.collezioni ? select['collezioni'] : '<?php echo my_wpml_string('Scegli una collezione', 'catellani'); ?>')"></span>
		...
		<div class="projects__select" click-outside="isSelect['tipologie']=false" ng-class="{'projects__select--filled' : projects.tipologie}" ng-click="isSelect['tipologie']=!isSelect['tipologie']">
			<span class="projects__value" ng-bind-html="(projects.tipologie ? select['tipologie'] : '<?php echo my_wpml_string('Scegli una tipologia', 'catellani'); ?>')"></span>
			<span class="projects__icons">
				<i class="icon-select"></i>
		...
		<div class="projects__select" click-outside="isSelect['periodi']=false" ng-class="{'projects__select--filled' : select['periodi']!=false}" ng-click="isSelect['periodi']=!isSelect['periodi']">
			<span class="projects__value" ng-bind-html="(select['periodi'] ? select['periodi'] : '<?php echo my_wpml_string('Scegli un periodo', 'catellani'); ?>')"></span>
			<span class="projects__icons">
				<i class="icon-select"></i>
			...
			<ul class="projects__options" ng-class="{'projects__options--visible':isSelect['periodi']}">
				<li class="projects__option" ng-click="$event.stopPropagation();select['periodi']='<?php echo $year; ?>';beforeAfter({before : '<?php echo $year; ?>', after : false})" ng-class="{'projects__option--selected':select['periodi']=='<?php echo $year; ?>'}"><?php echo my_wpml_string('Pre', 'catellani'); ?> <?php echo $year; ?></li>
				...
				<li class="projects__option" ng-click="$event.stopPropagation();select['periodi']='<?php echo my_wpml_string('Post 2015', 'catellani'); ?>';beforeAfter({after : '2015', before : false})" ng-class="{'projects__option--selected':select['periodi']=='<?php echo my_wpml_string('Post 2015', 'catellani'); ?>'}"><?php echo my_wpml_string('Post 2015', 'catellani'); ?></li>
			</ul>
		</div>
	</div>
	<div class="projects__items projects__items--grow-lg projects__items--shrink-fw projects__items--grid">
		<span class="projects__cell projects__cell--found" ng-if="items.length <= 0 && firstLoad" ng-class="{'projects__cell--found-visible':firstLoad}"><span><?php echo my_wpml_string('Nessun risultato trovato'); ?></span></span>
