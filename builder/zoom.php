<div class="zoom zoom--shrink-fw" ng-click="isZoom[<?php echo $row; ?>]=true;updateScrollbar(isZoom[<?php echo $row; ?>], <?php echo get_sub_field('zoom_image')['width'] ?>, <?php echo get_sub_field('zoom_image')['height']; ?>)" ng-class="{'zoom--active' : isZoom[<?php echo $row; ?>]}" ng-zoom id="zoom_<?php echo $row; ?>" ng-mousemove="cursor($event)">
	...
	<div class="zoom__container">
		<figure class="zoom__figure" ng-style="{'background-image' : 'url(<?php echo get_sub_field('zoom_preview') ? get_sub_field('zoom_preview')['url'] : get_sub_field('zoom_image')['url']; ?>)'}">
			...
			<span class="zoom__close zoom__close--shrink zoom__close--grow-md" ng-click="$event.stopPropagation();isZoom[<?php echo $row; ?>]=false">
