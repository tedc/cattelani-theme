...
<form class="glossary-search glossary-search--shrink glossary-search--gray glossary-search--grow-md" name="glossaryForm">
	<div class="glossary-search__box glossary-search__box--mw" glossary-autocomplete>
		...
		<ul class="glossary-search__results" ng-class="{'glossary-search__results--visible':glossary.isSearch}">
			<li class="glossary-search__item glossary-search__item--searching" ng-if="!glossary.items.length || glossary.isLoading" ng-bind-html="(glossary.isLoading) ? '<?php _e('Cerca (minimo due caratteri)...', 'catellani'); ?>' : '<?php _e('Nessun termine trovato...', 'catellani'); ?>'"></li>
...
<ul class="glossary__terms glossary__terms--shrink-fw">
	...
	<li class="glossary__term glossary__term--mw" id="<?php echo $term->term_id; ?>">
		<a href="<?php echo get_term_link($term->term_id); ?>" class="glossary__link" ui-sref="app.glossary({name : '<?php echo $term->slug; ?>', lang : '<?php echo ICL_LANGUAGE_CODE; ?>'})"><span>[<?php echo $term->term_order; ?>] / <?php echo strtolower($term->name); ?></span></a>
	</li>
	<?php endforeach; ?>
</ul>
</div>
