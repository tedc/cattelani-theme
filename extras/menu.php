    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if(! empty($state)) {
            if($state == 'popup') {
                ...
                $sref =  ' ui-sref="app.'.$state.'({lang : \''.get_field('lingua', $item->ID).'\''.$page.'})" ng-class="{\''.$prefix . $suffix['link'].'--active\':menuItem == \''.(get_field('state', $item->ID) == 'page') ? str_replace(esc_attr(home_url('/')), '', esc_attr($item->url) ) : '' .'\'}"';
