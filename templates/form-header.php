<?php
    ...
    if($_POST) :
        ...
        if(!wp_verify_nonce( $nonce, $string )) $fields_not_set[] = "nonce";
        ...
        if(empty($fields_not_set)) {
            ...
            if( $mMailer->send($mEmail) && $mMailer->send($rEmail)){
                echo "<alert-container><h3 class='form__title'>".$sent."</h3><p>".$tnx."</p><a ui-sref='app.root({lang : \"".ICL_LANGUAGE_CODE."\"})' class='form__send' href='".get_home_url()."'>".__('Torna alla home', 'catellani')."</a></alert-container>";
            } else {
