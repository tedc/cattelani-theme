	...
	<div class="search">
		<div class="search__filters search__filters--grid search__filters--grow-top"> 
			<div class="search__select" click-outside="isSelect['collezioni']=false" ng-class="{'search__select--filled' : search.collezioni}" ng-click="$event.stopPropagation();isSelect['collezioni']=!isSelect['collezioni']">
				<!-- <span class="search__value" ng-bind-html="(search.collezioni ? select['collezioni'] : '<?php echo my_wpml_string('Collezione', 'catellani'); ?>')"></span> -->
				<span class="search__value"><?php echo my_wpml_string('Collezione', 'catellani'); ?></span>
			...
			<div class="search__select" click-outside="isSelect['posizioni']=false" ng-class="{'search__select--filled' : search.posizioni}" ng-click="isSelect['posizioni']=!isSelect['posizioni']">
				<!-- <span class="search__value" ng-bind-html="(search.posizioni ? select['posizioni'] : '<?php echo my_wpml_string('Tipologia', 'catellani'); ?>')"></span> -->
				<span class="search__value"><?php echo my_wpml_string('Tipologia', 'catellani'); ?></span>
				<span class="search__icons">
					<i class="icon-select"></i>
				</span>
			...
			<div class="search__select" click-outside="isSelect['fonti']=false" ng-class="{'search__select--filled' : search.fonti}" ng-click="isSelect['fonti']=!isSelect['fonti']">
				<!-- <span class="search__value" ng-bind-html="(search.fonti ? select['fonti'] : '<?php echo my_wpml_string('Fonte luminosa', 'catellani'); ?>')"></span> -->
				<span class="search__value"><?php echo my_wpml_string('Fonte luminosa', 'catellani'); ?></span>
				<span class="search__icons">
					<i class="icon-select"></i>
				</span>
			...
			<div class="search__select" click-outside="isOrder=false" ng-click="isOrder=!isOrder">
				<span class="search__value"><?php echo my_wpml_string('Ordina', 'catellani'); ?>: {{orderValue()}}</span>
				<span class="search__icons">
					<i class="icon-select"></i>
				</span>
		...
		<div class="search__results" ng-if="select.collezioni.length > 0 || select.posizioni.length > 0 || select.fonti.length > 0">
			<div class="search__result" ng-if="select.collezioni.length > 0"><span><?php echo my_wpml_string('Collezioni'); ?>:</span> <strong ng-repeat="c in select.collezioni">{{($index > 0) ? ', ' : ' '}}{{c}} <i class="icon-close" ng-click="clear(c)"></i></strong></div>
			<div class="search__result" ng-if="select.posizioni.length > 0"><span class="search__sep" ng-if="select.collezioni.length > 0">&mdash;</span><span><?php echo my_wpml_string('Tipologie', 'catellani'); ?>:</span> <strong ng-repeat="p in select.posizioni">{{($index > 0) ? ', ' : ' '}}{{p}} <i class="icon-close" ng-click="clear( p)"></i></strong></div>
			<div class="search__result" ng-if="select.fonti.length > 0"><span class="search__sep" ng-if="select.collezioni.length > 0 || select.posizioni.length > 0">&mdash;</span><span><?php echo my_wpml_string('Fonti'); ?>:</span> <strong ng-repeat="f in select.fonti">{{($index > 0) ? ', ' : ' '}}{{f}} <i class="icon-close" ng-click="clear(f)"></i></strong></div>
		...
		<div class="search__items search__items--shrink-fw" scrollbar="search" speed="1.5">
			<p class="search__cell search__cell--found search__cell--grow-md" ng-if="filtered() && isLoading">
				<span><?php echo my_wpml_string('Nessuna lampada per la ricerca effettuata', 'catellani'); ?></span>
			</p>
