<div class="modal__container modal__container--search" collection-search lang="<?php echo ICL_LANGUAGE_CODE; ?>" id="modal-search">
	<div class="search">
		...
		<div class="search__results" ng-if="select.collezioni.length > 0 || select.posizioni.length > 0 || select.fonti.length > 0">
			...
			<div class="search__result" ng-if="select.fonti.length > 0"><span class="search__sep" ng-if="select.collezioni.length > 0 || select.posizioni.length > 0"></span><span><?php _e('Fonti'); ?>:</span> <strong ng-repeat="f in select.fonti">{{($index > 0) ? ', ' : ' '}}{{c}} <i class="icon-close" ng-click="clear(f)"></i></strong></div>
